...
- Write-tool results carry a `url:` line pointing at what was created, so you can open it without leaving the terminal: commits get theirs from the configured remote (GitHub, GitLab, Bitbucket, Gitea / Codeberg, or any https / ssh / scp-like remote), the GitHub tools from `gh` / API responses. No remote, no URL — the line is simply omitted.
